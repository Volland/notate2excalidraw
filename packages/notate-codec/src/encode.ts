...
function regionKey(id: RegionId): string {
  return `${id.x}_${id.y}`;
}

function strokeToProto(s: StrokeItem): Record<string, unknown> {
  const n = s.points.length;
  const packed = new Array<number>(n * PACKED_STRIDE);
...
function textToProto(t: TextItem): Record<string, unknown> {
  return {
    text: t.text,
    x: t.x,
    y: t.y,
...
function imageToProto(im: ImageItem): Record<string, unknown> {
  return {
    uri: im.uri,
    x: im.x,
    y: im.y,
...
function linkToProto(l: LinkItem): Record<string, unknown> {
  return {
    label: l.label,
    target: l.target,
    x: l.x,
...
function newBucket(id: RegionId): RegionBucket {
  return {
    id,
    strokes: [],
    images: [],
...
function growBounds(bucket: RegionBucket, item: NotateItem): void {
  const b = itemBounds(item);
  if (b.left < bucket.left) bucket.left = b.left;
  if (b.top < bucket.top) bucket.top = b.top;
  if (b.right > bucket.right) bucket.right = b.right;
  if (b.bottom > bucket.bottom) bucket.bottom = b.bottom;
}

/** Serialize a {@link NotateDoc} to ZIP bytes readable by the notate app. */
export function writeNotate(doc: NotateDoc): Uint8Array {
  const regionSize = doc.meta.regionSize || 4096;
  const buckets = new Map<string, RegionBucket>();

  const bucketFor = (item: NotateItem): RegionBucket => {
  ...
  // Region files + index.
  const indexRegions: Record<string, unknown>[] = [];
  for (const bucket of buckets.values()) {
    ...
    indexRegions.push(
      RegionBoundsProto.toObject(
        RegionBoundsProto.create({
          idX: bucket.id.x,
          idY: bucket.id.y,
  ...
  files['index.bin'] = RegionIndex.encode(
    RegionIndex.create({ regions: indexRegions }),
  ).finish();

  // Manifest. Ensure a uuid so notate's session resume logic has a stable id.
  const uuid = doc.meta.uuid ?? randomUuid();
  files['manifest.bin'] = CanvasData.encode(
    CanvasData.create({
